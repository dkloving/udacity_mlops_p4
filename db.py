...
class ProjectDB:
    ...
    def insert_dataset(cls, input_filenames, dataset):
        ...
        cls._cursor.execute("INSERT INTO datasets VALUES(?, ?, ?);", (next_id, input_filenames, dataset))
    ...
    def get_latest_dataset(cls):
        ...
        dset = {
            'id': latest_dataset[0],
            'file_list': latest_dataset[1].split(','),
            'data': pd.read_csv(io.StringIO(latest_dataset[2]), index_col=0)
        }
        return dset
